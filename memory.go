...
// init adds the memory store to the providerMap
// on initialisation of the app.
func init() {
	providers.RegisterProvider(MemoryStore, func(cfg Config) provider {
		return &memory{gocache.New(DefaultExpiry, DefaultCleanup), cfg}
	})
}
